...
let DB_URL_FALLBACK = "mysql://avnadmin:AVNS_k_BWki3ZEZI7Gpb8kTc@smartbuy-mihirchaudhari1621-ddd4.d.aivencloud.com:25692/defaultdb?ssl-mode=REQUIRED";

// Append connection timeouts to handle serverless cold starts and sleeping DBs
// Append connection timeouts to handle serverless cold starts and sleeping DBs.
// Free tier Aiven often has limit of ~5 connections, so we limit pool to 1 per lambda to be extremely conservative.
// We also add pgbouncer=true to hint at transaction mode if applicable, though for mysql it's less critical but good practice.
const connectionParams = "&connect_timeout=60&pool_timeout=60&socket_timeout=60&connection_limit=2";
