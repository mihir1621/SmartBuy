...
export default function ProductReviews({ product }) {
    ...
    return (
        <section className="mt-16 bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" id="reviews">
            {/* Tabs Header */}
            <div className="flex border-b border-gray-100">
                <button
                    onClick={() => setActiveTab("reviews")}
                    className={`flex-1 py-4 text-center font-bold text-sm uppercase tracking-wider border-b-2 transition-colors ${activeTab === "reviews" ? "border-black text-black" : "border-transparent text-gray-400 hover:text-gray-600"}`}
                >
                    Reviews ({reviews.length})
                </button>
                <button
                    onClick={() => setActiveTab("details")}
                    className={`flex-1 py-4 text-center font-bold text-sm uppercase tracking-wider border-b-2 transition-colors ${activeTab === "details" ? "border-black text-black" : "border-transparent text-gray-400 hover:text-gray-600"}`}
                >
                    Product Details
                </button>
            </div>
            ...
            <div className="p-6 md:p-8">
                <AnimatePresence mode="wait">
                    {activeTab === "reviews" ? (
                        ...
                        >
                            {/* Summary Header */}
                            <div className="flex flex-col md:flex-row gap-8 items-center justify-between mb-10 pb-10 border-b border-gray-100">
                                <div className="flex items-center gap-6">
                                    <div className="text-center">
                                        ...
                                        <div className="text-sm text-gray-400">{product.reviews} Verification</div>
                                    </div>
                            ...
                            <AnimatePresence>
                                {showForm && (
                                    <motion.form
                                        ...
                                        className="bg-gray-50 p-6 rounded-xl mb-8 overflow-hidden"
                                    >
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div>
                                                ...
                                                <input required type="text" className="w-full px-4 py-2 rounded-lg border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500" placeholder="Summarize your experience" value={newReview.title} onChange={e => setNewReview({ ...newReview, title: e.target.value })} />
                                            </div>
                                            <div>
                                                <label className="block text-sm font-medium mb-1">Review</label>
                                                <textarea required rows="3" className="w-full px-4 py-2 rounded-lg border border-gray-200 outline-none focus:ring-2 focus:ring-blue-500" placeholder="What did you like or dislike?" value={newReview.content} onChange={e => setNewReview({ ...newReview, content: e.target.value })}></textarea>
                                            </div>
                            ...
                            <div className="space-y-6">
                                {reviews.map((review) => (
                                    <div key={review.id} className="border-b border-gray-50 last:border-0 pb-6 last:pb-0">
                                        ...
                                        <button className="flex items-center gap-1.5 text-xs text-gray-500 hover:text-blue-600 transition-colors group">
